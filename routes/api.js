...
router.route('/login').post(function (req, res) {
    ...
    db.query('SELECT * FROM user WHERE Email = ' + db.escape(userLogin.email), (error, results, fields) => {
        if (error) throw error;
        if (results[0]) {
            bcrypt.compare(userLogin.password, results[0].Password, function (err, same) {
                if (same) {
